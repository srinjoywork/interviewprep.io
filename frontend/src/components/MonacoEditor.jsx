...
const MonacoEditor = ({
  ...
  const handleCodeExecution = async () => {
    setOutputValue(""); // Reset output before execution

    if (!selectedLanguage?.id || !code) {
      setOutputValue("Please provide code and select a language.");
      return;
    }
    ...
    const formData = {
      ...
      source_code: btoa(code), // Encode source code
      stdin: btoa(customInput || ""), // Encode input, default empty
    };
    ...
    try {
      ...
      const response = await axios.request({
        ...
        headers: {
          "x-rapidapi-key":
            "6fc790a7femsh121843a4d6a6a5ep114eb6jsnb43405cb7299",
      ...
      if (!token) {
        setOutputValue("Error: No token received.");
        return;
      }
  ...
  const checkStatus = async (token) => {
    const options = {
      method: "GET",
      url: `https://judge029.p.rapidapi.com/submissions/${token}`, // Updated to process.env
      params: {
        base64_encoded: "true",
        fields: "*",
      },
      headers: {
        "x-rapidapi-key": "6fc790a7femsh121843a4d6a6a5ep114eb6jsnb43405cb7299",
        "x-rapidapi-host": "judge029.p.rapidapi.com",
      },
    };
    ...
    try {
      ...
      // If code is still processing, retry after 2 seconds
      ...
      const decodedOutput = response.data.stdout
        ? atob(response.data.stdout)  // Decode stdout
        : response.data.stderr
        ? atob(response.data.stderr)  // Decode stderr (if any)
        : "No output received.";
  
      console.log("Decoded Output:", decodedOutput);
      setOutputValue(decodedOutput);  // Set readable output
