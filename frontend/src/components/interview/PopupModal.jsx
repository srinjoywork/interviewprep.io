...
function PopupModal() {
  const { setStatus, peerId, roomId, setRoomId, socket } = useContext(DataContext);
  const [isOpen, setIsOpen] = useState(false);
  const navigate = useNavigate();

  // Set Room ID only when peerId is available
  useEffect(() => {
    if (isOpen && peerId) {
      setRoomId(peerId);
    }
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              <FaCopy
                className="text-2xl text-gray-500 hover:text-gray-800 transition-all cursor-pointer"
                onClick={() => {
                  if (roomId) {
