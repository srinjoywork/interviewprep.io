...
class VideoChat extends Component {
    constructor(props) {
        ...
        this.state = {
            videoSocket: null,
            pc: null,
            controls: { video: true, audio: true },
            peerConnected: false,
            gotMediaDevice: false,
        };
    }
    ...
    componentDidMount() {
        ...
        videoSocket.addEventListener("open", () => console.log("Connected to video server"));

        const pc = VideoHelper.peerConnectionInit(videoSocket);
        pc.ontrack = (event) => {
        ...
        navigator.mediaDevices
            ...
            .then((stream) => {
                ...
                this.setState({ gotMediaDevice: true });
            })
            .catch((err) => {
                console.error("Error accessing media devices:", err);
            });

        videoSocket.addEventListener("message", this.handleSocketMessage);
    }
