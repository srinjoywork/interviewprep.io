class TextDiffBinding {
  ...
  onInput = (newValue, e) => {
    if (!e || !e.changes || e.changes.length === 0) {
      console.warn("Editor change event is missing expected properties:", e);
      return;
    }
  
    let previous = this.doc.data?.[this.path[0]] || ""; // Ensure `previous` is always a string
    let value = newValue; // Monaco Editor considers new line as \r\n.
    ...
    let rangeOffset = e.changes?.[0]?.rangeOffset || start; // Ensure rangeOffset is always valid
  
    if (previous.length !== start + end) {
      let removed = previous.slice(start, previous.length - end);
      this._remove(start, removed, e?.changes?.[0]?.rangeOffset ?? 0);
    }
    if (value.length !== start + end) {
      let inserted = value.slice(start, value.length - end);
      this._insert(start, inserted, e?.changes?.[0]?.rangeOffset ?? 0);
    }
